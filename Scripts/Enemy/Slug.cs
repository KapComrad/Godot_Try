...
public class Slug : KinematicBody2D, IDamagable
{
    ...
    [Export]
    private float _gravityScale = 98.1f;
    ...
    [Export]
    private bool _enabledPath = true;
    ...
    public void Attack(Node2D player)
    {
        if (player is Player)
        {
            ((Player)player).TakeDamage(
                _damage,
                player.GlobalPosition.DirectionTo(GlobalPosition),
                _force
            );
        }
    }
    ...
    private void Move()
    {
        ...
        if (
            (!_earthDetectorRay.IsColliding() && IsOnFloor())
            || (_wallDetectorRay.IsColliding() && IsOnFloor())
        )
        {
